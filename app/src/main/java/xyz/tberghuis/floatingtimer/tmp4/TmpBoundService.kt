...
private class TmpBoundService private constructor(private val application: Application) {
  ...
  companion object {
    @Volatile
    private var INSTANCE: TmpBoundService? = null
    fun getInstance(context: Context): TmpBoundService {
      synchronized(this) {
        return INSTANCE ?: TmpBoundService(context.applicationContext as Application).also {
          INSTANCE = it
        }
      }
    }
...
//val Context.boundService: BoundService
//  get() = BoundService.getInstance(this)

suspend fun Context.provideFloatingService(): FloatingService {
  return TmpBoundService.getInstance(this).provideService()
}